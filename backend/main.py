...
# ── prediction import (graceful fallback) ────────────────────────────────────
try:
...
def predict_patient_route(req: PatientPredictIn, username: str = Depends(get_current_user)):
    ...
    # PQRST confidence adjustment (mirrors original logic)
    pqrst = extra["pqrst"]
    adj = 0
    if pqrst.get("qt_interval")  and pqrst["qt_interval"]  > 440: adj += 5
    if pqrst.get("qrs_duration") and pqrst["qrs_duration"] > 120: adj += 3
    if pqrst.get("pr_interval")  and pqrst["pr_interval"]  > 200: adj += 2
    if adj > 0:
        if res["prediction"] == 1:
        ...
        res["all_proba"]   = ([round(100-nc, 1), round(nc, 1)]
                               if res["prediction"] == 1
